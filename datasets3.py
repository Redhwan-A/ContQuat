...
class AFLW2000(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # return img, torch.tensor(q), labels, self.X_train[index]
        # return img, torch.tensor(q).clone().detach(), labels, self.X_train[index]
        return img, q.clone().detach(), labels, self.X_train[index]
        # return img, torch.FloatTensor(q), labels, self.X_train[index]
...
class BIWI(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Get target tensors
        cont_labels = torch.FloatTensor([yaw, pitch, roll])
        # return img, torch.tensor(q), cont_labels, self.X_train[index]
        return img, q.clone().detach(), cont_labels, self.X_train[index]
...
class Pose_300W_LP(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Get target tensors
        R = utils.get_q(pitch, yaw, roll)#+ noise
