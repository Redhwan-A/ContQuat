...
def quat_self_supervised_primal_loss(q, A, reduce=True):
    # print('q', q, q.shape)
    # print('A', A, A.shape)
    # q = q.unsqueeze(1).unsqueeze(1)  # Shape: (3, 1, 1, 4)
...
def quat_consistency_loss(qs, q_target, reduce=True):
    ...
    # print("Shapes - q: {}, q_inv: {}, q_target: {}".format(q.shape, q_inv.shape, q_target.shape))
...
#==============================================================================================================
def quaternion_mse_loss(q, q_target, reduce=True):
    assert (q.shape == q_target.shape)
    d = quat_norm_diff(q, q_target)

    # Calculate MSE loss
    mse_loss = nn.MSELoss()
    loss = mse_loss(d, torch.zeros_like(d))  # Assuming the target is zeros (or whatever is appropriate)

    if reduce:
        loss = torch.mean(loss)

    return loss
# def quaternion_mse_loss(q, q_target, reduce=True): #Redhwan
#     """
...
# def quaternion_geodesic_loss1(q1, q2, reduce=True):
#     assert q1.shape == q2.shape and q1.shape[-1] == 4
#
#     dot_product = quaternion_dot(q1, q2)
#     dot_product = torch.clamp(dot_product, min=-1.0, max=1.0)  # Ensure the dot product is within [-1, 1]
#
#     geodesic_distance = 2.0 * torch.acos(torch.abs(dot_product))
#
#     if reduce:
...
#     dot_products = torch.clamp(dot_products, -1.0, 1.0)  # Ensure dot_products are within valid range
#
...
# def quaternion_geodesic_loss(q_pred, q_true): #Redhwan for quaternions
#     """
...
#     dot_product = torch.clamp(torch.sum(q_pred * q_true, dim=-1), -1.0, 1.0)
#
