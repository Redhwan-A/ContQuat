...
class RotationNet(nn.Module):
    def __init__(self,
        ...
        self.linear_reg = nn.Linear(fea_dim, 10) ##Outputs Bx10
        # print('self.linear_reg (Bx10):', self.linear_reg)
    ...
    def forward(self, x):
        ...
        # print('x (Bx10):', x)
        q = qcqp_layers.A_vec_to_quat(x)
        # print('q(Bx4):', q)
        return q

class QuatNet(torch.nn.Module):
    def __init__(self, backbone_name, backbone_file, deploy, pretrained=True,enforce_psd=True, unit_frob_norm=False):
    ...
    def forward(self, x):
        ...
        # print('q(Bx4):', q)
        if self.enforce_psd:
            x = convert_Avec_to_Avec_psd(x)
        if self.unit_frob_norm:
            x = normalize_Avec(x)
        # print ('xxxxxxxxxxxxx', x)
        q = self.qcqp_solver(x)
